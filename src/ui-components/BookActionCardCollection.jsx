...
import { listBookActionCards } from "../graphql/queries";
...
const nextToken = {};
const apiCache = {};
export default function BookActionCardCollection(props) {
  ...
  const loadPage = async (page) => {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await API.graphql({
          query: listBookActionCards.replaceAll("__typename", ""),
          variables,
        })
      ).data.listBookActionCards;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "BookActionCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
